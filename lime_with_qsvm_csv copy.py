...
def load_nsl_kdd_robust(train_file, test_file, max_samples=5000):
    """
    Robust NSL-KDD dataset loading with dynamic column handling
    LIMITED to max_samples for faster computation
    """
    try:
        print("📁 Loading NSL-KDD dataset from local files...")
        print(f"⚠️  LIMITING DATASET TO {max_samples} SAMPLES FOR FASTER COMPUTATION")
        ...
        if len(train_df) > max_samples:
            ...
            print(f"📉 Training data limited to {max_samples} samples")
        
        # Limit test data proportionally (about 25% of training size)
        test_max = max_samples // 4
        if len(test_df) > test_max:
            test_df = test_df.sample(n=test_max, random_state=42)
            print(f"📉 Test data limited to {test_max} samples")
...
def generate_dummy_data():
    ...
    n_components = 4
    train_size = 5000  # Reduced for faster computation
    test_size = 250
...
def main():
    print("=" * 70)
    print("   🚀 OPTIMIZED QSVM - LIMITED TO 5000 SAMPLES FOR SPEED")
    print("=" * 70)
    qsvm_start = time.time()

    # Load and preprocess data with 5000 sample limit
    train_df, test_df = load_nsl_kdd_robust('KDDTrain+.csv', 'KDDTest+.csv', max_samples=5000)
    ...
    try:
        ...
        # Use smaller subsets for kernel computation if needed
        if len(X_train_reduced) > 5000:
            # Further reduce for kernel computation if still too large
            train_indices = np.random.choice(len(X_train_reduced), 5000, replace=False)
            X_train_kernel = X_train_reduced[train_indices]
            y_train_kernel = y_train[train_indices]
        else:
            X_train_kernel = X_train_reduced
            y_train_kernel = y_train
            
        train_kernel = compute_quantum_kernel_advanced(X_train_kernel, X_train_kernel, n_qubits)
        test_kernel = compute_quantum_kernel_advanced(X_test_reduced, X_train_kernel, n_qubits)
    ...
    grid = GridSearchCV(SVC(kernel='precomputed', probability=True), param_grid, cv=5, scoring='f1')
    grid.fit(train_kernel, y_train_kernel)
    ...
    def qsvm_predict_proba(X):
        ...
        if len(X.shape) == 1:
            X = X.reshape(1, -1)
        K = compute_quantum_kernel_advanced(X, X_train_kernel, n_qubits)
        return svm_quantum.predict_proba(K)

    # Create LIME explainer
    feature_names = [f'Principal Component {i+1}' for i in range(X_train_kernel.shape[1])]
